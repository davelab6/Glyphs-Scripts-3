...
class viewNLI(ReporterPlugin):
	...
	def settings(self):
		...
		self.menuName = Glyphs.localize({'en': u'NLI Viewer', 'de': u'Mein Plugin'})
		# Load slider view as a right-click context menu
		self.generalContextMenus = [{'name': 'NLI Viewer', 'view': self.sliderView}]
	...
	def sliderUpdate(self):
		...
		for i in range(len(self.font.axes)):
			# if keySlider != None:
			# 	keySliderAlias = vars(viewNLI)['slider%s' % (keySlider)])
			# sliderAlias = vars(viewNLI)['slider%s' % (i + 1)]
			if globals()['check%s' % (i + 1)] == True:
...
# Start sync layers defs ———————————————————————————————————————————————————————————————————————
	...
	def syncToControl(self, aName, cName, dName, bName, NLIName):	
		currentGlyph = Glyphs.font.selectedLayers[0].parent
		...
		for node in currentGlyph.layers[0].paths[0].nodes:	
			...
			currentGlyph.layers[NLIName].paths[node.index].nodes[0].position = currentGlyph.layers[aName].paths[0].nodes[node.index].position
			currentGlyph.layers[NLIName].paths[node.index].nodes[1].position = currentGlyph.layers[cName].paths[0].nodes[node.index].position
			currentGlyph.layers[NLIName].paths[node.index].nodes[2].position = currentGlyph.layers[dName].paths[0].nodes[node.index].position
			currentGlyph.layers[NLIName].paths[node.index].nodes[3].position = currentGlyph.layers[bName].paths[0].nodes[node.index].position


	# Sync everything to the respective NLI layer V2
	# needs to work with more paths*****
	def syncToNLI(self, aName, cName, dName, bName, NLIName):	
		currentGlyph = Glyphs.font.selectedLayers[0].parent
...
# End sync layers defs —————————————————————————————————————————————————————————————————————————
	...
	# This should work in tandem with the NLI selection to allow multiple axes to be previewed at the same time
	...
	def foreground(self, layer):
		...
		# if re.match("NLI$", layer.name) != None:
		# 	self.syncToNLI("Regular", "C1", "D1", "B", "B NLI")
		# 	self.syncToNLI("Regular Oblique", "E1", "F1", "G", "G NLI")
		# 	self.syncToNLI("Bold", "H1", "I1", "J", "J NLI")
		# 	self.syncToNLI("Bold Oblique", "K1", "L1", "M", "M NLI")
		# else:
		# 	self.syncToControl("Regular", "C1", "D1", "B", "B NLI")
		# 	self.syncToControl("Regular Oblique", "E1", "F1", "G", "G NLI")
		# 	self.syncToControl("Bold", "H1", "I1", "J", "J NLI")
		# 	self.syncToControl("Bold Oblique", "K1", "L1", "M", "M NLI")
